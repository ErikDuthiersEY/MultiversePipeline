...
def build_eval_pipeline(env_path:str, datasets_path:str, config_path:str, compute:str):
    ...
    rule_based_metrics = command(
        ...
        environment=env_path,
        command="python run_rule_based_metrics.py --raw_dir ${{inputs.raw_dir}} --out_dir ${{outputs.out_dir}} --config ${{inputs.config}}",
    ...
    aggregate = command(
        ...
        environment=env_path,
        command="python run_aggregate.py --scores_dir ${{inputs.scores}} --out_dir ${{outputs.out_dir}}",
    ...
    def _pipeline(datasets_folder: Input, config_file: Input) -> None:
        ...
        rule_based_metrics_job = rule_based_metrics(raw_dir=infer_job.outputs.out_dir, config=config_file)
