...
def build_eval_pipeline(env: Environment, datasets_path: str, config_path: str, compute: str):
    ...
    inference = command(
        ...
        inputs={
            "datasets": Input(type="uri_folder"),
    ...
    rule_based_metrics = command(
        ...
        environment=env,
        command=(
            "python run_rule_based_metrics.py "
    ...
    llm_based_metrics = command(
        ...
        environment=env,
        command=(
            "python run_llm_evaluated_metrics.py "
            "--raw_dir ${{inputs.raw_dir}} "
            "--out_dir ${{outputs.out_dir}} "
            "--config ${{inputs.config}}"
        ),
    ...
    aggregate = command(
        ...
        command=(
            ...
            "--scores_dir ${{inputs.rule_scores}} "
            "${{inputs.llm_scores}} "
            "${{inputs.thput_scores}} "
            "--out_dir ${{outputs.out_dir}}"
        ),
        inputs={
            "rule_scores": Input(type="uri_folder"),
            "llm_scores": Input(type="uri_folder"),
            "thput_scores": Input(type="uri_folder"),
        },
        outputs={"out_dir": Output(type="uri_folder")},
    )
    ...
    def _pipeline(datasets_folder: Input, config_file: Input) -> None:
        ...
        rule_based_metrics_job = rule_based_metrics(
            raw_dir=infer_job.outputs.out_dir,
            datasets=datasets_folder,
            config=config_file,
        )

        llm_based_metrics_job = llm_based_metrics(
            raw_dir=infer_job.outputs.out_dir,
            config=config_file,
        )

        thput_metrics_job = thput_metrics(
            datasets=datasets_folder,
            config=config_file,
            raw_dir=infer_job.outputs.out_dir 
        )
        ...
        agg_job = aggregate(
            rule_scores=rule_based_metrics_job.outputs.out_dir,
            llm_scores=llm_based_metrics_job.outputs.out_dir,
            thput_scores=thput_metrics_job.outputs.out_dir,
        )
